    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-200">
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-[#1e4d8c]" />
          <h3 className="font-bold text-slate-900 text-sm">Filter Parts</h3>
          {activeFilterCount > 0 && <span className="px-1.5 py-0.5 bg-[#1e4d8c] text-white text-[10px] font-bold rounded-full">{activeFilterCount}</span>}
        </div>
      ...
      <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
        <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5"><Car className="w-3.5 h-3.5 text-[#1e4d8c]" />Vehicle Fitment</span>
        {activeVehicle ? (
          <label className="flex items-start gap-2.5 cursor-pointer mt-1 pt-1 border-t border-slate-200/80">
            <input type="checkbox" checked={filters.onlyFitsVehicle} onChange={e => onFilterChange({ ...filters, onlyFitsVehicle: e.target.checked })} className="mt-0.5 rounded-sm text-[#1e4d8c] focus:ring-[#1e4d8c] cursor-pointer" />
          ...
          <div className="text-xs text-slate-600 space-y-2 pt-1">
            ...
            <button type="button" onClick={() => openSelectorModal('vin')} className="w-full py-1.5 px-2 bg-[#1e4d8c] hover:bg-[#0d1f3c] text-white font-bold text-xs rounded-md flex items-center justify-center gap-1 cursor-pointer transition-colors"><Car className="w-3.5 h-3.5" /><span>Select Vehicle</span></button>
          </div>
        )}
      </div>
          ...
          {!collapsed.partTypes && <div className="space-y-1.5 mt-2">
            {Object.entries(partTypeCounts).map(([type, count]) => (
              <label key={type} className="flex items-center justify-between text-xs text-slate-700 hover:text-slate-900 cursor-pointer py-0.5 group">
                <div className="flex items-center gap-2"><input type="checkbox" checked={filters.partTypes.includes(type)} onChange={() => toggleArr('partTypes', type)} className="rounded-sm text-[#1e4d8c] focus:ring-[#1e4d8c] cursor-pointer" /><span className="group-hover:text-[#1e4d8c] transition-colors">{type}</span></div>
          ...
          {!collapsed.positions && <div className="space-y-1.5 mt-2">
            {Object.entries(positionCounts).map(([pos, count]) => (
              <label key={pos} className="flex items-center justify-between text-xs text-slate-700 cursor-pointer py-0.5 group">
                <div className="flex items-center gap-2"><input type="checkbox" checked={filters.positions.includes(pos)} onChange={() => toggleArr('positions', pos)} className="rounded-sm text-[#1e4d8c] focus:ring-[#1e4d8c] cursor-pointer" /><span className="group-hover:text-[#1e4d8c] transition-colors">{pos}</span></div>
                <span className="text-[10px] text-slate-400 font-mono bg-slate-100 px-1.5 rounded-xs">{count}</span>
              </label>
            ))}
          </div>}
          ...
          {!collapsed.brands && <div className="mt-2 space-y-2">
            {Object.keys(brandCounts).length > 4 && (
              <div className="relative">
                <input type="text" value={brandSearch} onChange={e => setBrandSearch(e.target.value)} placeholder="Search brands..." className="w-full pl-7 pr-2 py-1 text-[11px] bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#1e4d8c]" />
            ...
            <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1">
              {filteredBrands.map(brand => (
                <label key={brand} className="flex items-center justify-between text-xs text-slate-700 cursor-pointer py-0.5 group">
                  <div className="flex items-center gap-2"><input type="checkbox" checked={filters.brands.includes(brand)} onChange={() => toggleArr('brands', brand)} className="rounded-sm text-[#1e4d8c] focus:ring-[#1e4d8c] cursor-pointer" /><span className="group-hover:text-[#1e4d8c] transition-colors font-medium">{brand}</span></div>
      ...
      <div>
        <label className="flex items-center gap-2 text-xs font-bold text-slate-800 cursor-pointer">
          <input type="checkbox" checked={filters.onlyInStock} onChange={e => onFilterChange({ ...filters, onlyInStock: e.target.checked })} className="rounded-sm text-[#1e4d8c] focus:ring-[#1e4d8c] cursor-pointer" />
      ...
      <div className="fixed inset-0 z-50 flex flex-col justify-end bg-slate-900/60 backdrop-blur-xs md:hidden animate-in fade-in duration-150">
        <div className="bg-white rounded-t-2xl max-h-[85vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-200">
          <div className="p-4 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2"><Sliders className="w-4 h-4 text-[#1e4d8c]" /><h3 className="font-bold text-slate-900 text-sm">Filter Results</h3></div>
          ...
          <div className="p-4 border-t border-slate-200 bg-slate-50">
            <button type="button" onClick={onCloseMobile} className="w-full py-2.5 bg-[#1e4d8c] hover:bg-[#0d1f3c] text-white font-bold text-sm rounded-lg shadow-sm cursor-pointer">Apply Filters</button>
          </div>
        </div>
      </div>
    );
