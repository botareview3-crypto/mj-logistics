      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-[72px]">
          ...
          <div className="hidden md:flex items-center gap-3 shrink-0">
            <button type="button" onClick={() => go(currentUser ? '/account' : '/signin')} className="text-sm font-semibold text-white/80 hover:text-white px-3 py-2 transition-colors cursor-pointer">
      ...
      {isMobileOpen && (
        <div className="md:hidden border-t border-white/10 bg-[#0B1220]">
          <div className="px-4 sm:px-6 py-3 flex flex-col">
            ...
            <button type="button" onClick={() => go(currentUser ? '/account' : '/signin')} className="text-left py-3 text-base font-semibold text-white/70 border-b border-white/5 cursor-pointer">
              {currentUser ? 'My Account' : 'Sign In'}
            </button>
