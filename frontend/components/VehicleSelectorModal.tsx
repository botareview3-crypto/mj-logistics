...
/* Spring variants — modal emerges from where the trigger was (center of screen),
   scales from 0.93 → 1.0. Interruptible: AnimatePresence handles reverse.     */
...
export const VehicleSelectorWidget: React.FC<{ isEmbedded?: boolean; className?: string }> = ({
  isEmbedded = false,
  className = '',
}) => {
  const {
    activeVehicle, setActiveVehicle, addSavedVehicle,
    savedVehicles, selectorInitialTab, closeSelectorModal, showToast,
  } = useApp();
  ...
  const handleConfirmCascading = () => {
    ...
    const newVehicle: Vehicle = {
      id: currentEngineObj.id, make: selectedMake, model: selectedModel,
      generation: selectedGen, yearRange: currentGenObj.yearRange,
      year: currentGenObj.startYear + 2, engine: currentEngineObj.name,
      engineCode: currentEngineObj.code, powerHp: currentEngineObj.powerHp,
      fuelType: currentEngineObj.fuelType, nickname: `${selectedMake} ${selectedModel}`,
    };
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden ${className}`}>
      ...
      {/* Tabs — pointer-down feedback */}
      <div className="flex border-b border-gray-200 bg-white">
        {(['vin', 'cascading'] as const).map(tab => (
          <motion.button
            key={tab}
            type="button"
            onClick={() => { setActiveTab(tab); setDecodeError(null); }}
            className={`
      ...
      {/* Tab indicator — spring-animated underline */}
      {/* (Already handled by border-b-2 above — spring on tab switch via AnimatePresence) */}
        ...
        {activeTab === 'vin' && (
          ...
          >
            {!decodedCandidate ? (
              <div className="space-y-4">
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-slate-800 mb-1.5">
                    Enter Vehicle Registration Number or 17-digit VIN:
                  </label>
                  <div className="flex flex-col sm:flex-row gap-2">
                    <div className="relative flex-1">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span className="bg-amber-400 text-slate-950 font-mono font-bold text-xs px-1.5 py-0.5 rounded border border-amber-500">GB</span>
                      </div>
                      <input
                        id="reg-vin-input"
                        type="text"
                    ...
                    >
                      {isDecoding
                        ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /><span>Decoding...</span></>
                        : <><Search className="w-4 h-4" /><span>Find Parts</span></>
                      }
                    </motion.button>
                  </div>
                  {decodeError && (
                    <div className="mt-2 text-xs text-rose-600 flex items-center gap-1.5 font-medium">
                      <AlertCircle className="w-4 h-4 shrink-0" /><span>{decodeError}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-2 border-t border-slate-100">
                  <div className="text-xs text-slate-500 font-medium mb-2 flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-amber-500" />Try popular sample registrations:
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {SAMPLE_VIN_REG_DATABASE.slice(0, 4).map(sample => (
                      <motion.button
                        ...
                        transition={SPRINGS.micro}
                      >
                        <span className="font-bold">{sample.vehicle.regNumber}</span>
                        <span className="text-slate-400 font-sans">({sample.vehicle.make} {sample.vehicle.model})</span>
                      </motion.button>
                    ))}
                  </div>
                </div>
              ...
              /* Decoded candidate — spring entrance */
              <motion.div
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-2 pt-1">
                  ...
                  >
                    ...
                    <span>Yes, this is my car — Confirm &amp; Filter Parts</span>
                  </motion.button>
                  <motion.button
                    type="button"
                    onClick={() => { setDecodedCandidate(null); setDecodeError(null); }}
                    className="py-2.5 px-4 bg-white hover:bg-slate-100 text-slate-700 font-semibold text-sm rounded-lg border border-slate-300 flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
                    whileTap={{ scale: 0.97 }}
                    transition={SPRINGS.micro}
                  >
        ...
        {activeTab === 'cascading' && (
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">1. Select Make <span className="text-rose-500">*</span></label>
                <select value={selectedMake} onChange={e => handleMakeChange(e.target.value)} className={selectClass}>
                  <option value="">-- Choose Make --</option>
                  {VEHICLE_MAKES_DATABASE.map(m => <option key={m.name} value={m.name}>{m.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">2. Select Model <span className="text-rose-500">*</span></label>
                <select disabled={!selectedMake} value={selectedModel} onChange={e => handleModelChange(e.target.value)} className={selectedMake ? selectClass : disabledSelectClass}>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">3. Year / Generation <span className="text-rose-500">*</span></label>
                <select disabled={!selectedModel} value={selectedGen} onChange={e => handleGenChange(e.target.value)} className={selectedModel ? selectClass : disabledSelectClass}>
                  <option value="">{selectedModel ? '-- Choose Generation / Year --' : 'Select Model first'}</option>
                  {availableGenerations.map(g => <option key={g.name} value={g.name}>{g.name} ({g.yearRange})</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">4. Select Engine <span className="text-rose-500">*</span></label>
                <select disabled={!selectedGen} value={selectedEngineId} onChange={e => setSelectedEngineId(e.target.value)} className={selectedGen ? selectClass : disabledSelectClass}>
                  <option value="">{selectedGen ? '-- Choose Engine --' : 'Select Generation first'}</option>
                  {availableEngines.map(e => <option key={e.id} value={e.id}>{e.name} — ({e.fuelType}, code {e.code})</option>)}
                </select>
              </div>
            </div>
            <div className="pt-2 flex items-center justify-between gap-3">
              <div className="text-xs text-slate-500">
                {currentEngineObj
                  ? <span className="text-emerald-700 font-semibold flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" />Ready to lock in fitment</span>
                  : <span>Complete all 4 dropdowns to filter catalog</span>
                }
              </div>
              <motion.button
                type="button"
      ...
      {savedVehicles.length > 0 && (
        <div className="bg-slate-50/90 border-t border-slate-200 px-4 sm:px-6 py-2.5 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 text-slate-600 font-medium">
            <History className="w-3.5 h-3.5 text-slate-400" /><span>Saved in My Garage:</span>
          </div>
            ...
            {savedVehicles.map(v => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() => {
                    setActiveVehicle(v);
                    showToast(`Active vehicle switched to ${v.make} ${v.model}`, 'success');
                    if (!isEmbedded) closeSelectorModal();
                  }}
                  className={`px-2 py-0.5 rounded font-semibold border transition-colors cursor-pointer ${
                    isActive ? 'bg-emerald-100 text-emerald-900 border-emerald-300' : 'bg-white text-slate-700 hover:bg-slate-100 border-slate-200'
                  }`}
                  whileTap={{ scale: 0.95 }}
                  transition={SPRINGS.micro}
                >
...
/* ── Modal wrapper — spring entrance/exit, proper translucent backdrop ──── */
export const VehicleSelectorModal: React.FC = () => {
  const { isSelectorModalOpen, closeSelectorModal } = useApp();

  /* Close on Escape — works even mid-animation */
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') closeSelectorModal();
    };
  ...
  return (
    <AnimatePresence>
      {isSelectorModalOpen && (
        /* Backdrop — translucent dark scrim, spring fade */
        <motion.div
          key="vehicle-modal-backdrop"
        ...
        >
          {/* Panel — spring scale+translate entrance, same path on exit */}
          <motion.div
            key="vehicle-modal-panel"
            variants={panelVariants}
            initial="hidden"
            animate="visible"
            exit="exit"
            transition={SPRINGS.sheet}
            className="w-full max-w-2xl max-h-[calc(100vh-1.5rem)] overflow-y-auto bg-white rounded-2xl shadow-2xl border border-slate-200 relative"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div className="px-5 py-4 bg-slate-900 text-white flex items-center justify-between rounded-t-2xl">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 id="vehicle-selector-title" className="text-base sm:text-lg font-bold">Select your vehicle</h2>
                  <p className="text-xs text-slate-300">Use your vehicle details to check part compatibility</p>
                </div>
              </div>
              <motion.button
                type="button"
